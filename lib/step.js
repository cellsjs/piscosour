...
Step.prototype.report = function(result) {
  if (!_.get(global, `${this.name}.${this._context}`, false)) {
    _.set(global, `${this.name}.${this._context}`, {});
  }

  let report = global[this.name][this._context].xxreport;
  if (!report) {
    report = {
      name: this.name,
  ...
  global[this.name][this._context].xxreport = report;
};

          ...
          Object.getOwnPropertyNames(cooked.recipes[recipeName].flows[flowName].steps).forEach((stepName) => {
            if (cooked.recipes.module.config.implementations[stepName]) {
              Object.getOwnPropertyNames(cooked.recipes.module.config.implementations[stepName]).forEach((context) => {
                if (context === 'multiple') {
                  cooked.recipes.module.config.implementations[stepName][context].contexts.forEach((subContext) => {
                    logger.trace('add requires from implementations ->', '#cyan', recipeName,'flow:', '#green', flowName, 'step:', '#green', stepName, 'context:', '#cyan', subContext);
                    cooked.recipes[recipeName].flows[flowName].steps[stepName][subContext] = {
                      requires : cooked.recipes.module.config.implementations[stepName][context].requires
                    }
                  });
                } else {
                  ...
                  cooked.recipes[recipeName].flows[flowName].steps[stepName][context] = {
                    requires : cooked.recipes.module.config.implementations[stepName][context]
                  }
                }
              });
            }

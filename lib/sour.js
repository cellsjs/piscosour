...
module.exports = function() {
  ...
  const normalize = function(command) {
    ...
    if (normal.repoType) {
      const exists = config.repoTypes.find(function(unit) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const execute = function(normal) {
        ...
        if (normal.isShot) {
          ...
          analytics.hit(`/shots/${normal.repoType}:${normal.name}/do`, `shot: ${normal.repoType}::${normal.name}`);
          shooter.execute(normal, _resolve, _reject);
        } else {
          analytics.hit(`/straws/${normal.repoType}:${normal.name}/do`, `straw: ${normal.repoType}:${normal.name}`);

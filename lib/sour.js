...
module.exports = function() {
   ...
   *
   * @returns {{name: *, context: *, orig: *, recipe: *, isShot: *}}
   * @private
   */
  const normalize = function(command) {
    ...
    normal.context = !normal.context ? (!config.context ? config.defaultType : config.context) : normal.context;
    ...
    return new Promise((resolve, reject) => {
      ...
      const execute = function(normal) {
        ...
        if (normal.isShot) {
          ...
          analytics.hit(`/shots/${normal.context}::${normal.name}`, `shot: ${normal.context}::${normal.name}`);
          shooter.execute(normal, _resolve, _reject);
        } else {
          analytics.hit(`/straws/${normal.context}:${normal.name}`, `straw: ${normal.context}:${normal.name}`);

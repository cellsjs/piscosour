...
const getStepParams = function(normal) {
  ...
  const applyMerger = (paramsStep) => {
    const params = _.cloneDeep(paramsStep);
...
const getFlowParams = function(normalStep, normalFlow) {
  ...
  const paramsSteps = _.fromPairs(_.map(normalSteps, normalStep => [normalStep.context, _mergeObject(_getStep(normalStep), flow.params)]));
  ...
  const applyMergers = (normalStep, paramsStep) => {
    let merger = mergers[normalStep.context] || [];
  ...
  const finalParams = _.fromPairs(_.map(normalSteps, normalStep => [normalStep.context, _precededParams(applyMergers(normalStep, paramsSteps[normalStep.context]))]));  

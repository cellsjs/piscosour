 ...
 * @returns {string}
 */
var getModulesDir = function(recipes){
    var modulesDir = {};
    ...
    if (fs.existsSync(file)) {
        ...
        if (pkg.dependencies)
            for (var name in pkg.dependencies){
                ...
                else if (pkgModule.name!== "piscosour" && pkgModule.keywords && pkgModule.keywords.indexOf("piscosour-recipe")>=0){
                    logger.trace(name,"is a piscosour recipe --> ", pkgModule.version);
                    recipes.push({name:name, version: pkgModule.version, description: pkgModule.description});
                }
            }
    }
...
var getConfig = function() {
    ...
    var recipes = getRecipes(pwd);

    var config = {
        recipes : recipes,
        modulesDir : getModulesDir(recipes),
        warnings : [],
        merge : mergeObject,
        rootDir : pwd
    };

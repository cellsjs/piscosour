...
var getConfig = function() {
    ...
    var _getGenAll = function(normal, type){
        logger.silly("#green","config:_getGenAll -in-", normal, type);
        var obj;
        for (var name in config.recipes){
            var recipe = config.recipes[name];
            if (recipe[type] && recipe[type][normal.name]){
                obj = recipe[type][normal.name];
                break;
            }
        }
        logger.silly("#green","config:_getGenAll -out-", obj);
        return obj;
    };

    // ----------------

    var config = scullion.cook(chef.getRecipes());
    logger.trace("Scullion ends","#green","Config is cooked:",JSON.stringify(config));

    var mergedConfig = _mergeConfig(config.recipes);
    ...
    var _precesedParams = function(params){
    ...
    var loadOne = function(normal, recipe, shot){
        ...
        if (fs.statSync(filename)) {
            ...
            shot.runner.params = cmdParams.merge(normal.params);
        }

...
const addImplementations = (_mergedConfig) => {
  if (_mergedConfig.implementations) {
    Object.getOwnPropertyNames(_mergedConfig.flows).forEach((flowName) => {
      Object.getOwnPropertyNames(_mergedConfig.flows[flowName].steps).forEach((stepName) => {
        if (_mergedConfig.implementations[stepName]) {
          Object.getOwnPropertyNames(_mergedConfig.implementations[stepName]).forEach((context) => {
            logger.trace('add requires from implementations ->', 'flow:', '#green', flowName, 'step:', '#green', stepName, 'context:', '#cyan', context);
            _mergedConfig.flows[flowName].steps[stepName][context] = {
              requires: _mergedConfig.implementations[stepName][context] === constants.notBuild ? constants.notBuild : versionUtils.line2Requires(_mergedConfig.implementations[stepName][context])
            };
          });
        }
      });
...
const instantiate = function() {
  ...
  mergedConfig = addImplementations(_mergeConfig(config.recipes));

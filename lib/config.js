...
const _getUniqueStraw = function(strawsIn, normal) {
  ...
  if (straws.length > 1) {
    let error = 'straw: "' + normal.name + '" Ambiguous for type "' + normal.context + '"!! shots has the same name in multiple recipes: ';
    straws.forEach((pair) => {
 ...
 *
 * ```
 * normal = {name: *, context: *, orig: *, recipe: *, isShot: *}
 * ```
...
const _getShot = function(normalIn) {
  ...
  if (globalShot) {
    ...
    shot = _mergeObject(shot, globalShot[normal.context]);
...
const getStrawParams = function(normalShot, normal) {
  ...
  const mergers = [
    ...
    _.get(straw, `shots.${normalShot.name}.${normalShot.context}.params`, false),
...
const _loadShot = function(normal) {
  ...
  const found = Object.getOwnPropertyNames(config.recipes)
    .reverse()
    .map((recipeName) => {
      const stepContext = _.get(config.recipes[recipeName], `shots.${normal.name}.${normal.context}`, null);

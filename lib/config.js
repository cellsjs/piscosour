...
var Config = function() {
    ...
    var load = function(normal){
        ...
        for (var name in config.recipes){
            ...
            if (recipe.name){
                ...
                shot = loadPlugins(normalCopy,recipe,shot);
            }
        }
    ...
    var loadPlugins = function(normal, recipe, shot){
        if (normal.params.plugins && normal.params.plugins.length>0) {
            for (var i in normal.params.plugins) {
                ...
                if (fsUtils.exists(filename)) {
                    shot.plugins[name] = _mergeObject(require(filename), shot.plugins[name]);
                    shot.plugins[name].setLogger(shot.logger);
                    shot.plugins[name].setParams(shot.runner.params);
                    if (shot.plugins[name].runner.addons){
                        shot.setAddons(shot.plugins[name].runner.addons);
                    }
                }

                logger.silly("#green", "config:loadPlugin -out-", shot.plugins?shot.plugins[name]:"");
            }
        }
        return shot;
    };

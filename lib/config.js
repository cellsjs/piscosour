...
const getFlowParams = function(normalStep, normalFlow) {
  ...
  const normalSteps = _.chain(contexts)
    .map(context => normalByContext(context))
    .fromPairs().value();

  const paramsByContext = normalStep_ => [
    normalStep_.context,
    _mergeObject(_getStep(normalStep_), flow.params)
  ];

  const paramsSteps = _.chain(normalSteps)
    .map(normalStep_ => paramsByContext(normalStep_))
    .fromPairs().value();

  const priorityMergers = context => [
    context,
    [
  ...
  const priorityMergersByContext = _.chain(contexts)
    .map(context => priorityMergers(context))
    .fromPairs().value();
  ...
  const finalParams = _.chain(normalSteps)
    .map(normalStep_ => [normalStep_.context, _precededParams(applyPriorities(normalStep_, paramsSteps[normalStep_.context]))])
    .fromPairs().value();

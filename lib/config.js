  ...
  const found = Object.getOwnPropertyNames(config.recipes)
    .map((recipeName) => {
      ...
      if (!stepContext && !stepDefault) {
        ...
        logger.trace('#green', 'config:_loadShot', `shot [${context ? context : 'default'}::${normal.name}] found in recipe: ${recipeName}`);
        logger.trace('#green', 'config:_loadShot module:', normal._module);

        if (tempStep) {
          logger.warn('#yellow', 'shot', tempStep.name, '#red', 'is overwritten by shot:', '#bold', normal.origName, 'in recipe:', recipeName);
        }
        tempStep = step;
      ...
      .map((recipeName) => {
        ...
        if (plugin._module) {
          ...
          if (plugins[pluginName]) {
            logger.warn('#yellow', 'plugin', pluginName, '#red', 'is overwritten by:', '#bold', found[0]._module, 'from recipe:', recipeName);
          }

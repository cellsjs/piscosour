...
var Config = function () {
    ...
    var _mergeUnit = function (config, recipes, name) {
        var recipe = recipes[name];
        config = _mergeObject(config, recipe.config);
        config.shots = _mergeObject(config.shots, recipe.shots);
        config.straws = _mergeObject(config.straws, recipe.straws);
        ...
        if (recipe.config && recipe.config.straws) {
            ...
            config.straws = _mergeObject(config.straws, recipe.config.straws);
        }
    ...
    var getShotInfo = function (name) {
        ...
        for (var recipeName in config.recipes) {
            ...
            if (recipe.name && recipe.shots[name]) {
                info.description = _getDescription(path.join(recipe.dir, "shots", name, "shot.js"), info.description);
                info.recipes.push({name: recipe.name, version: recipe.version});
                for (var type in recipe.shots[name]) {
                    info.types.push(type === 'default' ? 'all' : type);
                    info.description = _getDescription(path.join(recipe.dir, "shots", name, type, "shot.js"), info.description);
                }
            }
        }
        return info;
    ...
    var _getDescription = function (file, description) {
        if (fsUtils.exists(file)) {
            var desc = require(file).description;
            if (desc !== description)
                description += desc;
        }
        return description;
    ...
    var load = function (normal) {
        ...
        if (shot.do)
            shot = setPlugins(shot, normalCopy);
    ...
    var setPlugins = function (shot, normal) {
        var plugins = {};
        for (var recipeName in config.recipes) {
            var recipe = config.recipes[recipeName];
                ...
                for (var i in normal.params.plugins) {
                    ...
                    if (fsUtils.exists(filename)) {
                        ...
                        if (plugins[name].addons) {
                            ...
                            shot.augment(plugins[name].addons);
                        }
                    }

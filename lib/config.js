...
const getStepParams = function(normal) {
  ...
  const normalsByContext = _.chain(contexts)
    .map(context => [context, _.merge(_.clone(normal), {context: context})])
    .fromPairs().value();

  const stepsByContext = _.chain(normalsByContext)
    .map(normalStep => [normalStep.context, _getStep(normalStep)])
    .fromPairs().value();
  ...
  const finalParams = _.chain(normalsByContext)
    .map(normalStep => [normalStep.context, _precededParams(applyMerger(stepsByContext[normalStep.context]))])
    .fromPairs().value();
...
const getFlowParams = function(normalStep, normalFlow) {
  ...
  const normalsByContext = _.chain(contexts)
    .map(context => [
      context,
      _.chain(normalStep).clone().merge({ context: context }).value()
    ])
    .fromPairs().value();

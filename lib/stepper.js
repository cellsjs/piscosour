...
const stepper = function() {
  ...
  const execNormal = function(normal, resolve, reject) {
    ...
    if (step._do) {
      ...
      const message = `Step "${normal.name}" is not defined for context: "${normal.context}"`;
      const report = reporter.ensureStepReport(normal.name, normal.context, `${normal.context}::${normal.name}`);
      const result = {
        status: normal.checkImpl ? 2 : 0,
      ...
      reporter.saveReport(reporter.addReportResult(report, result));
      if (normal.checkImpl) {

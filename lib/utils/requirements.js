...
const _getVersion = (cmd, result, options, father) => {
  ...
  let version = result.output ? result.output.replace(os.EOL, '') : undefined;
  if (regexp) {
    const match = result.output.match(regexp);
    if (match && match.length > 1) {
      version = match[1].replace(os.EOL, '');
    } else {
      version = `regexp: ${regexp}, version: ${version.substring(0, version.length > 20 ? 20 : version.length)}`;
    }
  }
  return version;
};
const _cachedExec = (command, noCache) => {
  ...
  if (!result) {
    ...
    return launcher.execute(cmds[0], cmds.slice(1), {mute: true, stdio: ['ignore', 'pipe', 'pipe']})
      .then((result) => {

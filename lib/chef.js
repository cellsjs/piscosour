...
const chef = function() {
  ...
  const addRecipe = (recipes, rootDir, isGlobal) => {
  ...
  const getRecipes = function(isGlobal) {
    ...
    if (fsUtils.exists(file)) {
      ...
      if (pkg.dependencies) {
        Object.getOwnPropertyNames(pkg.dependencies).forEach((name) => addRecipe(recipes, path.join(rootDir, 'node_modules', name), isGlobal));
      }
      if (isGlobal) {

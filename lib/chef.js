...
const chef = function() {
  ...
  const addRecipe = (recipes, rootDir, isGlobal, isLocal) => {
    ...
    const pkgPiscoRangeVersion = pkgModule.devDependencies && pkgModule.devDependencies.piscosour ?
      pkgModule.devDependencies.piscosour : pkgModule.dependencies && pkgModule.dependencies.piscosour ? pkgModule.dependencies.piscosour : '';
    ...
    if (pkgModule.keywords && pkgModule.keywords.indexOf('piscosour-recipe') >= 0 && recipes.module.name !== pkgModule.name) {
      if (pkgModule.name === 'piscosour' && isLocal || semver.satisfies(piscoVersion, pkgPiscoRangeVersion)) {
        ...
        if (isGlobal && pkgModule.dependencies) {
          Object.getOwnPropertyNames(pkgModule.dependencies).forEach((name) => addRecipe(recipes, path.join(rootDir, 'node_modules', name), isGlobal, isLocal));
        }

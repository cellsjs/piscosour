...
var docs = {
    ...
    auto: function (cb) {
        ...
        let workingDir = params.workingDir?params.workingDir:config.params.workingDir;
        var ami = context.whoami(workingDir);
    ...
    enrichCommands: function (ami, nocontext) {
        ...
        var mustBeInStraw = function(must, shots, recipeKey, type){
            for (var name in shots)
                if (must[recipeKey + "-" + name + "-" + type] || must[recipeKey + "-" + name + "-default"])
                    return true;
            ...
            if (recipe.name) {
                ...
                for (var name in recipe.shots) {
                    ...
                    for (var type in shot) {
                        ...
                        if (shot[type].mustBeIn)
                            mustBeInShots[recipeKey+"-"+name+"-"+type] = true;
                        if (!(shot.default ? shot.default.isTest : false)) {
                            if (!enriched[recipeKey])
                                enriched[recipeKey] = {___recipe: recipe};

                            if (nocontext || noAmi && !shot[type].mustBeIn || (ami && ami.indexOf(type)>=0) && shot[type].mustBeIn)
                ...
                for (var name in recipe.straws) {
                    ...
                    types.forEach((type) => {
                        var command = (type === 'default' ? 'all' : type) + ":" + name;
                        var mustBeIn = mustBeInStraw(mustBeInShots, straw.shots,recipeKey,type);

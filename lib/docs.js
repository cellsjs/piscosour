...
var docs = {
    ...
    whoami: function () {
        ...
        config.repoTypes.forEach((type)=> {
            logger.trace("checking if your are in a ", type, ":", context.is(type));
            if (context.is(type))
                ami.push(type);
        });
        return ami
            ...
            if (recipe.name) {
                for (var name in recipe.straws) {
                    ...
                    types.forEach((type) => {
                        ...
                        if (straw.type === 'normal' && (nocontext || noAmi && !straw.contextualized || (ami && ami.indexOf(type) >= 0) && straw.contextualized)) {
                ...
                for (var name in recipe.shots) {
                    ...
                    for (var type in shot) {
                        ...
                        if (!(shot.default ? shot.default.isTest : false)) {
                            ...
                            if (nocontext || noAmi && !shot[type].contextualized || (ami && ami.indexOf(type)>=0) && shot[type].contextualized)
                                enriched[recipeKey][command] = {

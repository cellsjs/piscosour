...
const docs = {
  ...
  cmdOptions: {
    ...
    'all': 'list all commands availables (context:straw[:step])',
  ...
  info: function(normal) {
    ...
    if (normal.isStep) {
      ...
      logger.parseMd(path.join(recipeDir, 'steps', normal.name, 'info.md'), path.join(recipeDir, 'steps', normal.name, normal.context, 'info.md'));
    } else {
      ...
      logger.txt('\n', chalk.bold.green('steps'), '(steps on the workflow)\n');
      const steps = [];
      for (const step in strawObj.steps) {
        if (strawObj.steps[step].type === 'straw') {
          const normalTmp = JSON.parse(JSON.stringify(normal));
          normalTmp.name = step;
          logger.txt('\n', chalk.yellow('## Straw ----'), '\n');
          logger.txt('', chalk.cyan.bold(step), 'is a straw');
          this.info(normalTmp);
          logger.txt('\n', chalk.yellow('## End straw ----'), '\n');
        } else {
          logger.txt('', chalk.cyan.bold(step + ':'), '\n');
          logger.parseMd(path.join(recipeDir, 'steps', step, 'info.md'), path.join(recipeDir, 'steps', step, normal.context, 'info.md'));
        }
      }
    }
  },
  ...
  /**
   * Return all the straws/steps available for the project.
  ...
  index: function() {
    ...
    docs.showSteps('Steps availables:', params.list === 'all' || params.list === 'steps');
    logger.txt('\n');
  },
  ...
  enrichCommands: function(ami, nocontext) {
    ...
    const mustBeInStraw = function(must, steps, type) {
        ...
        Object.getOwnPropertyNames(recipe.steps).forEach((name) => {
          ...
          Object.getOwnPropertyNames(step).forEach((type) => {
            ...
            if (!(step.default ? step.default.isTest : false)) {
              ...
              if (nocontext || noAmi && step[type].contextFree || (ami && ami.indexOf(type) >= 0) && !step[type].contextFree) {
    ...
    Object.getOwnPropertyNames(config.recipes).forEach((recipeKey) => {
      ...
      if (recipe.name && recipe.straws) {
        Object.getOwnPropertyNames(recipe.straws).forEach((name) => {
          ...
          types.forEach((type) => {
            const command = (type === 'default' ? 'all' : type) + ':' + name;
            const mustBeIn = mustBeInStraw(mustBeInSteps, straw.steps, type);
  ...
  showStraws: function(title, strawTypes, showSteps, isShown) {
    if (isShown) {
      ...
      if (Object.keys(straws).length > 0) {
        ...
        Object.getOwnPropertyNames(straws).forEach((name) => {
          ...
          if (straw.steps && showSteps) {
            logger.txt('\t', chalk.cyan('steps:'), Object.keys(straw.steps).toString(), '\n');
          }
        });
      }
    }

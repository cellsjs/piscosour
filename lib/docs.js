...
var params = require('./params'),
    ...
    pkgPisco = require(path.join(config.modulesDir.piscosour,'package.json')),
    chalk = require('chalk');
...
var docs = {
    ...
    index : function() {
        ...
        if (params.commands==="all")
            docs.showStrawCommands("Commands:",config.repoTypes,["normal", "parallel", "internal"]);
        else {
            var others = JSON.parse(JSON.stringify(config.repoTypes));
            if (config.defaultType!=="recipe")
                others.splice(others.indexOf("recipe"),1);
            docs.showStrawCommands("Commands:", others, ["normal", "parallel"]);
        }
        logger.txt("");
            ...
            for (var name in straws) {
                ...
                for (var i in types) {
                    if (repoTypes.indexOf(types[i]) >= 0) {
                        var msg = ["\t", chalk.bold((types[i] === config.defaultType ? "" : types[i] + ":") + name)];
                        if (straw.options)

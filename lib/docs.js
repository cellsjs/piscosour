      ...
      if (recipe.name && recipe.steps) {
        Object.getOwnPropertyNames(recipe.steps).forEach((name) => {
          ...
          Object.getOwnPropertyNames(step).forEach((type) => {
            ...
            const installed = !(step[type].requires && !step[type].installed);
      ...
      if (recipe.name && recipe.flows) {
        Object.getOwnPropertyNames(recipe.flows).forEach((name) => {
          ...
          types.forEach((type) => {
            ...
            const isInstalledCommand = (type, flow) => {
              const _or = (a, b) => a || b;
              const uninstalledStep = (step) => (step[type] !== undefined && !step[type].installed && step[type].requires);
              const uninstalledFlow = (flow) => _.reduce(_.map(flow.steps, uninstalledStep), _or);

              return !uninstalledFlow(flow);
            };
            const installed = isInstalledCommand(type, flow);

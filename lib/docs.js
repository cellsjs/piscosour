...
const docs = {
  ...
  _showShotMd: function(normal) {
    const tmp = config.getShotInfo(normal.name);
    if (tmp.recipes) {
      tmp.recipes.forEach((recipe) => {
        logger.parseMd(path.join(recipe.dir, 'shots', normal.name, 'info.md'), path.join(recipe.dir, 'shots', normal.name, normal.repoType, 'info.md'));
      });
    }
  },

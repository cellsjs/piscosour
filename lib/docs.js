...
const docs = {
  ...
  info: function(normal) {
    ...
    if (normal.isStep) {
      ...
      logger.parseMd(path.join(recipeDir, 'flows', normal.name, 'info.md'));
      logger.txt('\n', chalk.bold.green('steps'), '(steps on the workflow)\n');
      const steps = [];
      for (const step in flowObj.steps) {
  ...
  index: function() {
    ...
    docs.showFlows('Flows availables:', ['normal', 'parallel'], true, params.list === 'all' || params.list === 'flows');
    ...
    Object.getOwnPropertyNames(config.recipes).forEach((recipeKey) => {
      ...
      if (recipe.name && recipe.flows) {
        Object.getOwnPropertyNames(recipe.flows).forEach((name) => {
          ...
          types.forEach((type) => {
            const command = (type === 'default' ? 'all' : type) + ':' + name;
            const mustBeIn = mustBeInFlow(mustBeInSteps, flow.steps, type);

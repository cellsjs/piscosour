...
const docs = {
  ...
  showDisclaimer: function(cb) {
    ...
    const analytics = _.get(cfg, 'recipes.module.config.analytics')
    const pkgName = _.get(cfg, `recipes.userConfig.config.analytics.userok[${pkg.name}]`, true);

    if (analytics && pkgName) {
      ...
      const getPrompt = (enrich, command) => {
        return {
          ...
          name: enrich.installed 
            ...
            : `${command} ${chalk.green(`[install command]`)} ${chalk.yellow(` (${enrich.description})`)}`
        }
      };

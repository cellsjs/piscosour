...
var check = function() {
    ...
    var getJunit = function(init){
        ...
        junit.testsuites[0]._attr.time = (moment()-init)/1000;
    ...
    var shot2Testsuite = function(junit, report){
        ...
        testsuite.testsuite[0]._attr.time=report.time/1000;
        ...
        for (var i in report.results){
            ...
            testcase.testcase[0]._attr.time = result.time/1000;
            testcase.testcase[0]._attr.status = result.status;
                ...
                if (result.status!==0) {
                    if (result.content.keep) {
                        testcase.testcase.push(
                            {
                                faliure: [
                                    {_attr: {message: result.content.error}}
                        ...
                        testcase.testcase.push(
                            {
                                error: [
                                    {_attr: {message: result.content.error ? result.content.error : result.content}}
                                    ,(result.content.error ? result.content.error : result.content)
                                ]
                            });
                        testsuite.testsuite[0]._attr.errors++;
                        junit.testsuites[0]._attr.errors++;
                    }
                    testcase.testcase.push({"system-err": JSON.stringify(result.content)});
                }else{
                    if (result.content)

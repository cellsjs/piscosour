...
module.exports = (function() {
  ...
  const setStraw = function(junit, normal) {
    ...
    for (const name in straw.steps) {
      if (straw.steps[name].type === 'straw') {
        normal.name = name;
        setStraw(junit, normal);
      } else if (global[name] && global[name].xxreport) {
        junit.testsuites.push(step2Testsuite(junit, global[name].xxreport));
      }
    }
  };
  ...
  const getJunit = function(init) {
    ...
    junit.testsuites[0]._attr.name = 'Execution of ' + (params.normal.isStep ? 'step' : 'straw') + ': ( ' + config.cmd + ' ' + params.normal.context + ':' + params.normal.name + ' )';

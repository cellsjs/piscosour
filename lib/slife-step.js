...
var getStep = function(){
    ...
    var execute = function(normal,resolve,reject) {
        logger.trace("#green","slife-step:execute","piscosour task","#cyan",normal.orig," ----------- [","#yellow",messages["start"],"]----------");
    ...
    var synchro = function(n, resolve, reject){
        if (n<steps.length) {
            ...
            logger.trace("#green","slife-step:synchro","n-errors:","#cyan",errors.length,"n-shots:","#cyan",steps.length);
            if (errors.length<steps.length)
                resolve();
            else {
                logger.error("#green","slife-step:synchro","n-errors:","#cyan",errors.length,"n-shots:","#cyan",steps.length,"#red","errors",errors);
                reject(errors);
            }
        }
    };
    ...
    /**
     * writes the shots array in order to execute all the stages defined in configurations files
     * @param name
     */
    ...
    var load = function(normal){
        for (var name in config.modulesDir){
            var fileName = config.modulesDir[name]+"/shots/"+normal.name+"/task.js";

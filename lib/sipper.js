...
const sipper = function() {
  ...
  const synchro = function(normalFlow, n, resolve, reject) {
    ...
    const _addOutputs = function _addOutputs(params, mappings) {

      function getOutputValue(context, outputParameter, sourceStep) {
        return _.get(flow, `steps[${sourceStep}].outputs[${context}][${outputParameter}]`, '');
      }

      function setInputValue(context, inputParameter, outputValue) {
        const result = Object.create(null);
        result[context] = Object.create(null);
        result[context][inputParameter] = outputValue;
        
        return result;
      }

      function connectIO(outputParameter, sourceStep, inputParameter) {
        return _.chain(flow)
          .get(`steps[${sourceStep}].outputs`, {})
          .keys()
          .map((context) => {
            const outputValue = getOutputValue(context, outputParameter, sourceStep);
            return setInputValue(context, inputParameter, outputValue);
          })
          .flatten().value();
      }

      function connectIOs(outputMapping, inputParameter) {
        return _.chain(outputMapping)
          .map((outputParameter, sourceStep) => connectIO(outputParameter, sourceStep, inputParameter))
          .flatten().value();
      }

      const resultedParams = _.chain(mappings)
        .map(connectIOs)
        .flatten().value();
      
      let myParams = _.cloneDeep(params);
      resultedParams.forEach(resultedParam => myParams = _.merge(myParams, resultedParam));
  ...
  const filterContexts = function(step, normal){
    normal.context = normal.context.filter((context) => {
      ...
      if (!is) {
        logger.info('Run of', '#bold',  step.type ? step.type : 'step','#green', `"${normal.name}"`, 'is excluded for context', '#cyan', `"${context}"`);
      }
      return is;
    });

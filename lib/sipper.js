...
const sipper = function() {
  ...
  const synchro = function(normalFlow, n, resolve, reject) {
    ...
    const _resolve = function(outputs) {
      ...
      let exec = true;
      if (step.type === 'flow') {
        if (!finalCheck.run(normalFlow.init)) {
          exec = false;
    ...
    if (n < steps.length) {
      ...
      logger.info('Flow', '[', '#bold', normalFlow.orig, ']', 'finished -', '#duration', moment() - normalFlow.initFlow);
      resolve();
    }
  };
  ...
  const execute = function(normal, fromStep, toStep, resolve, reject) {
    ...
    logger.info('Starting flow: [', '#bold', normal.flowName, '], steps: [', '#bold', steps.join(', '), ']');

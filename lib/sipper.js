    ...
    const _addOutputs = function _addOutputs(params, mappings) {
      ...
      function connectIO(outputParameter, sourceStep, inputParameter) {
        return _.chain(flow)
          ...
          .keys()
          .map(context => setInputValue(context, inputParameter, getOutputValue(context, outputParameter, sourceStep)))
          .flatten()
          .value();
      }
      ...
      _.chain(resultedParams)
        .forEach(resultedParam => myParams = _.merge(myParams, resultedParam))
        .value();

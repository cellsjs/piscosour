...
var sipper = function() {
    ...
    var execute = function(normal, fromShot, toShot, resolve, reject) {
        ...
        logger.info("Executing straw","[","#bold",normal.orig,"]","shots:",shots);
    ...
    var synchro = function(normal, n,resolve,reject){
        if (n<shots.length) {
            ...
            var onReject = function (err) {
                logger.trace("#green", "sipper:synchro","#red","Shot Error Stops the straw!!");
                reject(err);
            ...
            if (shot.type==="straw")
                execute(normalShot,shot.from,shot.to,onResolve,onReject);
            else
                shotExec.execute(normalShot,onResolve,onReject);

        }else{
            logger.info("#green","sipper:synchro","#cyan",normal.name,"ended");
            resolve();
        }
    };

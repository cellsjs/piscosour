...
const sipper = function() {
  ...
  const synchro = function(normalFlow, n, resolve, reject) {
    ...
    const _addOutputs = function _addOutputs(params, mappings) {
      ...
      const backCompatibility = (_params) => {
        if (mappings){
          ...
          const getParams = (paramIn, paramOut, outputs) => {
            ...
            _.mapKeys(outputs, (contextMap, context) => {
              _.mapKeys(contextMap, (paramValue, key) => {
                if (paramOut === key) {
                  outParams[context][paramIn] = paramValue;
                }
                if (paramOut !== key || paramIn !== paramOut) {
                  delete outParams[context][key];
                }
              });
            });
            return outParams;
      ...
      let outParams =  _.chain(flow.steps)
        .map((step, stepName) => step.outputs)
        .map((paramTmp) => _.merge(myParams, paramTmp))
        .pop().value();

      outParams = backCompatibility(outParams);

      const diff = _.reduce(outParams, (result, value, key) => {
        return _.isEqual(value, params[key]) ? result : result.concat(key);
      }, []);
      diff.forEach((key) => {
        //console.log('from', params[key], '->', outParams[key]);
      });

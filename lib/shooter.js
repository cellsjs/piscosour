    ...
    var synchro = function(n, resolve, reject){
        ...
        var onResolve = function (result) {
            ...
            if (config.stages[n]) {
                ...
                shot.report({status: 0, message: config.stages[n], content: result, time: (end - shot.init)});
            }
        ...
        var onReject = function (err) {
            logger.error("#red", "shooter", "stop at", "#cyan", config.stages[n], "(", n, "of", config.stages.length, ")", JSON.stringify(err));
            var end = new Date().getTime();
            shot.report({status: 1, message: config.stages[n],content: err, time: (end-shot.init)});
            if (err && err.keep)
                synchro(n + 1, resolve, reject);
            else
                reject(err);

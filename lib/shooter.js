...
const shooter = function() {
  ...
  const execute = function(normal, resolve, reject) {
    const shotConfig = config.load(normal);

    shot = shotConfig.step ? new Shot(shotConfig.step, shotConfig.plugins) : {};
    ...
    if (shot._do) {
      ...
      reject('Shot "' + normal.name + '" is not defined for repository Type: "' + normal.context + '"');
    }
  };

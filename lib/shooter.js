...
var shooter = function(){
    ...
    var execute = function(normal,resolve,reject) {
        ...
        if (shot.do){
            logger.info("\n\n","#magenta","Starting","|","#green",shot.runner.description,"| [","#bold",normal.orig,"]","\n");
    ...
    var synchro = function(n, resolve, reject){
        ...
        var _resolve = function (result) {
            ...
            if (result && result.skip || isLast) {
                logger.info("\n\n","#magenta","Finished","|","#green",shot.runner.description,"-","#duration",moment()-shot.initShot,"\n");
                resolve();
            }else
        ...
        var _reject = function (err) {
            logger.error("#green", "shooter","error on", "#cyan", config.stages[n],"(", n, "of", config.stages.length, ")","#red","ERROR", JSON.stringify(err),err);
